...
public class HndTrck_record : MonoBehaviour
{
    // for hand tracking
    ...
    void Update()
    {
       ...
       // Debug.Log("EE Pos: " + EE_pos);
        ...
        else
        {
            ...
            if (Vector3.Magnitude(index_pos - thumb_pos) < 0.02)
            {
               string path = Path.Combine(Application.persistentDataPath, "DrawingCoordinates.txt");
                if (first_pinch)
                {
                    CreateLine(index_pos); //custom function 
                    using (StreamWriter writer = new StreamWriter(path, false))
                    {
                        string line = "\n finger pos:" + index_pos.ToString("R");
                   ...
                   // pathset = false;
                    UpdateLine(index_pos); //custom function 
                string line2 = "\n finger pos:" + index_pos.ToString("R");
...
public float DesAngle(Vector3 thumb_pos, Vector3 middle_pos, Vector3 thumb_prox_pos)
    ...
    void UpdateLine(Vector3 indexpos)
    {
        ...
        line_renderer.SetPosition(line_renderer.positionCount - 1, indexpos); // changed position to positions
    }
