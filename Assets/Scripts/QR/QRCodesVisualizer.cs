...
namespace QRTracking
{
    public class QRCodesVisualizer : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            if (confirm_button.activeSelf == true)
            {
                // start the QR tracking 
                gameObject.GetComponent<QRCodesManager>().StartQRTracking();
                HandleEvents();    // update QR position
                // place QR tracker and confirm button
                qrTracker.transform.position = SpatialGraphCoordinateSystem.QRpos;
                qrTracker.transform.rotation = SpatialGraphCoordinateSystem.QRrot;
                confirm_button.transform.position = SpatialGraphCoordinateSystem.QRpos + new Vector3(0.02f, -0.08f, -0.01f);
            }
        }
    }
}